...
// ★ 보안 패치: ADMIN_TOKEN 하드코딩 제거
// ScriptProperties에 'ADMIN_TOKEN' 키가 없으면 기존 기본값 사용 (하위 호환)
function getAdminToken() {
  return PropertiesService.getScriptProperties().getProperty('ADMIN_TOKEN') || 'samil_admin_2024';
}

// 최초 1회 실행 필요: GAS 편집기에서 initAdminToken() 함수를 직접 실행하세요
...
function handleRequest(p, body) {
  try {
    ...
    else if (action === 'changeAdminPw')   result = changeAdminPw(p);  // ★ 추가
...
// ★ 보안 패치: ADMIN_TOKEN 하드코딩 대신 getAdminToken() 사용
...
function changeAdminPw(p) {
  ...
  // 현재 비밀번호 검증 (토큰 또는 비밀번호 중 하나 일치하면 허용)
  ...
  // master 비밀번호로 로그인한 경우 masterPw 변경, common이면 commonPw 변경
  ...
  // ADMIN_TOKEN도 함께 변경 (새 비밀번호가 곧 새 토큰)
